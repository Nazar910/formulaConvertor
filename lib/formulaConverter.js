...
function translate(options, formula) {
    ...
    let replace = formula.replace(exponentiation, '$1<sup>$2</sup>');
    result = formula.search(exponentiation) === -1 ? formula : replace;

    //sin
    replace = result.replace(sin, 'sin $1');
    result = result.search(sin) === -1 ? result : replace;
    //cos
    replace = result.replace(cos, 'cos $1');
    result = result.search(cos) === -1 ? result : replace;
